describe('template spec', () => {
  //after each
  afterEach(() => {
    cy.exec('cd C:/xampp/htdocs/demo-app-cypress-automation && php artisan migrate:fresh --seed');
  });
  ...
  it.only('User can delete data user', () => {
    ...
    //  cy.get('.table td').contains('user').nextAll().contains('Delete').click();
    //  cy.get('.table td').contains('user').parent().contains('Delete').click();
    cy.get('.table td').contains('User Baru').parent().find('button').contains('Delete').click(); //paling aman menggunakan ini karena spesifik terdapat (button)
    //make sure sweet alert visible
    cy.get('.swal-button-container').find('button').contains('OK').click();
    //  cy.get(':nth-child(3) > .text-right > .d-flex > .ml-2 > .btn').click();
    //  cy.get(':nth-child(2) > .swal-button').click();
